...
This project now runs best through a small local Python server that serves the PWA and stores data in a SQLite database.
...
The SQLite database file is created automatically as `expense_tracker.db` in the project folder the first time the server starts.
...
Opening `index.html` directly still loads the UI, but SQLite storage requires running through `python server.py`. If the backend is unavailable, the app falls back to browser local storage for that session.
...
- Main storage is SQLite in `expense_tracker.db`
- The app keeps a browser local-storage fallback if the backend cannot be reached
...
- If the server is unavailable, the app temporarily falls back to browser local storage
