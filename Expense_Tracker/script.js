...
const API_BASE = '/api';
let backendEnabled = window.location.protocol.startsWith('http');
...
async function persistCurrentState() {
  ...
  try {
    ...
    console.error('Unable to persist data to SQLite backend, continuing with local storage only.', error);
  }
}
...
async function initializeData() {
  ...
  try {
    const backendBundle = await loadBundleFromBackend();
    backendEnabled = true;
    ...
    backendEnabled = false;
    console.error('SQLite backend unavailable, loading local storage fallback.', error);
    return localBundle;
  }
}
...
function importDatabase(file) {
  ...
  reader.onload = async (event) => {
    try {
      ...
      alert(backendEnabled ? 'Data imported and saved to SQLite successfully.' : 'Data imported successfully.');
    } catch (error) {
...
resetData.addEventListener('click', async () => {
  ...
  if (backendEnabled) {
    try {
      ...
      backendEnabled = false;
      console.error('Unable to reset SQLite data, reset completed locally only.', error);
    }
  }
  ...
  alert(backendEnabled
    ? 'Data reset in SQLite and local backup storage.'
